...
class OpenGazeTracker:
	...
	def __init__(self, ip='130.63.210.174', port=4242, logfile='default.tsv', \
		debug=False):
	...
	def _process_incoming(self):
		...
		while self._connected.is_set():
			...
			instring_decode = instring.decode()
			messages = instring_decode.split('\r\n')
	...
	def _process_outgoing(self):
		...
		while not self._sock_ready_for_closing.is_set():
			...
			t = time.time()
			messageEndode = msg.encode('utf-8')
			self._sock.send(messageEndode)
			# Unlock the socket again.
			self._socklock.release()
